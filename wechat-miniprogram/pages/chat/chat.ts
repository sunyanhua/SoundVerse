...
interface PageData {
  ...
  recordTime: number; // 录音时长（秒）
  recordFile: string | null; // 录音临时文件路径
  // 录音计时器
  recordTimer?: any;
}
...
Page({
   ...
   * 自动播放音频（延迟自动播放）- 改进版
   */
  autoPlayAudioDirectly(messageId: string, audioUrl: string) {
   ...
   * 上传录音文件进行语音识别
   */
  async uploadVoiceFile(tempFilePath: string) {
  ...
  async loadQuickQuestions() {
    try {
      // 从API获取建议
      const response = await get<{suggestions: string[]}>('/api/v1/chat/suggestions');
